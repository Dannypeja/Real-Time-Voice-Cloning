...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--datasets_root",
        type=str,
        help="Path to the mailabs root directory (e.g. '/training_data/de_DE').",
    )
    # overwrite
    # male, female, mix
    # audio format
    ...
    # list female speakers // changed \ to / fo work with unix
    ...
    for speaker in all_speakers:
        # get subfolders // changed \ to / fo work with unix
        ...
        for speaker_subfolder in speaker_subfolders:
            ...
            if last_folder == "wavs":
                ...
                # traverse further dirs // changed \ to / fo work with unix
                speaker_subfolders_books_search_dir = os.path.join(
                    speaker_subfolder, "*//"
                )
                speaker_subfolders_books_dirs = glob(
                    speaker_subfolders_books_search_dir
                )
                ...
                for speaker_subfolders_book in speaker_subfolders_books_dirs:
                    last_folder = os.path.basename(
                        os.path.normpath(speaker_subfolders_book)
                    )
                    if last_folder == "wavs":
